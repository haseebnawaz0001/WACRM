...
<template>
  <BaseNode :label="data?.label || 'Text'" header-class="bg-blue-600" :has-input="!data?.isEntryNode">
    ...
    <span v-if="inputType && inputType !== 'none'" class="inline-block mt-1 px-1.5 py-0.5 bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 rounded-md text-[10px] font-medium">
      {{ inputType }}
    </span>
  </BaseNode>
</template>
