...
<Aside type="caution">
  WA CRM stores templates as **flat fields** (`header_type`, `header_content`, `body_content`,
...
`IMAGE`, `VIDEO` or `DOCUMENT` — WA CRM wraps it into Meta's
...
Meta's four components map onto flat fields in the WA CRM API:

| Meta component | WA CRM fields |
|-----------|-------------|
...
WA CRM assembles the Meta `components` payload itself when you submit the template.
...
You do not set `parameter_format` yourself — WA CRM detects non-numeric placeholders and sends
...
<Aside type="note">
  WA CRM automatically detects if your template uses named parameters (non-numeric placeholders) and sets the appropriate format when submitting to Meta.
</Aside>
