...
<Aside type="caution">
  ...
  `media_url` field — the file is uploaded to WA CRM, which then sends it to Meta.
</Aside>
...
Meta's own size limits apply (roughly 5 MB images, 16 MB audio/video, 100 MB documents); WA CRM
