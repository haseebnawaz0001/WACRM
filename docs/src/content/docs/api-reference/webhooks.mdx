...
There are two distinct webhook systems in WA CRM:

1. **Outbound webhook subscriptions** (`/api/webhooks`) — endpoints **you** own
   that WA CRM calls when events happen (new message, contact created,
...
Register your own HTTPS endpoints to receive event notifications from WA CRM.
WA CRM signs each delivery, retries failures, and enforces SSRF protection on
the target URL.
...
When a secret is configured, WA CRM signs the raw request body and sends the
...
Every request carries `Content-Type: application/json` and `User-Agent: WACRM-Webhook/1.0`,
...
WA CRM exposes webhook endpoints that you configure in your Meta App settings:
...
WA CRM does this automatically — but **only when both** the `X-Hub-Signature-256` header is
