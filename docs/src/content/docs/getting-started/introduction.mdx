---
title: Introduction
description: Welcome to WA CRM - A modern WhatsApp Business Platform
---

import { Card, CardGrid } from '@astrojs/starlight/components';

<img src="/whatomate/images/dashboard-light.png" alt="WA CRM Dashboard" class="light-only" />
<img src="/whatomate/images/dashboard-dark.png" alt="WA CRM Dashboard" class="dark-only" />

## What is WA CRM?

WA CRM is a modern WhatsApp Business Platform built with Go (Fastglue) and Vue.js (shadcn-vue). It provides a comprehensive solution for businesses to manage their WhatsApp communications, automate responses, and engage with customers at scale.

## Key Features

<CardGrid>
  <Card title="Multi-tenant Architecture" icon="building">
    Support multiple organizations with isolated data and configurations.
  </Card>
  <Card title="Granular Roles & Permissions" icon="approve-check">
    Customizable roles with fine-grained, per-resource permissions. Assign specific permissions per resource (users, contacts, templates, etc.) and control access at the action level (read, write, delete, and more).
  </Card>
  <Card title="WhatsApp Cloud API" icon="rocket">
    Seamless integration with Meta's WhatsApp Business API.
  </Card>
  <Card title="Real-time Chat" icon="comment">
    Live messaging with WebSocket support for instant communication.
  </Card>
  <Card title="Template Management" icon="document">
    Create and manage message templates approved by Meta.
  </Card>
  <Card title="Bulk Messaging" icon="paper-plane">
    Send campaigns to multiple contacts efficiently.
  </Card>
  <Card title="Chatbot Automation" icon="setting">
    Keyword-based auto-replies, conversation flows, and AI-powered responses.
  </Card>
  <Card title="Voice Calling & IVR" icon="phone">
    Incoming and outgoing WhatsApp calls with IVR menus, DTMF routing, transfers to agent teams, hold music, and call recording.
  </Card>
  <Card title="Canned Responses" icon="pencil">
    Pre-defined quick replies with slash commands (`/shortcut`) and dynamic placeholders.
  </Card>
  <Card title="WhatsApp Flows" icon="puzzle">
    Build, publish, and send Meta's native interactive form flows.
  </Card>
  <Card title="Product Catalogs" icon="list-format">
    Sync catalogs and products from your Meta commerce account.
  </Card>
  <Card title="Teams & SSO" icon="approve-check">
    Route conversations and calls to agent teams; sign in via OAuth/OIDC providers.
  </Card>
  <Card title="Custom Actions" icon="external">
    Per-contact buttons that call your own webhooks or open external URLs.
  </Card>
  <Card title="Outbound Webhooks" icon="rocket">
    HMAC-signed event deliveries for messages, contacts, and transfers.
  </Card>
  <Card title="Audit Logs" icon="document">
    A per-organization trail of every create, update, and delete.
  </Card>
</CardGrid>

## Tech Stack

### Backend
- **Go 1.25+** with [Fastglue](https://github.com/zerodha/fastglue) (fasthttp-based HTTP framework)
- **PostgreSQL** for data storage with GORM v2
- **Redis** for caching and pub/sub
- **JWT** for authentication

### Frontend
- **Vue 3** with Composition API and TypeScript
- **Vite** for build tooling
- **shadcn-vue** / Radix Vue for UI components
- **TailwindCSS** for styling
- **Pinia** for state management
- **Vue Query** for server state

## Quick Links

<CardGrid>
  <Card title="Quickstart" icon="rocket">
    Get up and running in minutes. [Get started →](/whatomate/getting-started/quickstart)
  </Card>
  <Card title="API Reference" icon="open-book">
    Explore the REST API. [View API →](/whatomate/api-reference/overview)
  </Card>
</CardGrid>
