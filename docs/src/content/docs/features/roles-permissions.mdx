...
WA CRM provides a flexible role-based access control (RBAC) system that allows you to define custom roles with granular permissions.
...
| `tags` | read, write, delete, import, export | Contact tag management and bulk import/export |
| `contact_fields` | read, write, delete | The custom fields an organization adds to its contacts |
| `segments` | read, write, delete | Saved contact filters |
| `tasks` | read, write, delete | Follow-ups attached to contacts |
| `pipelines` | read, write, delete | Pipelines and their stages |
| `deals` | read, write, delete | Deals moving through a pipeline |
| `automations` | read, write, delete | Rules and their run history |
| `reports` | read, export | CRM reports; `export` allows the CSV download |
