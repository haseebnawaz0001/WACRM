...
Meta Insights surfaces the analytics that Meta computes for your WhatsApp Business account — messaging volume, conversation pricing, template performance, and calling stats. WA CRM proxies these live from the Meta Graph API and caches the results in Redis so repeated dashboard loads don't re-hit Meta on every view.

<Aside type="note">
  These are Meta's own aggregate analytics, distinct from WA CRM's internal dashboards (message counts, agent performance, chatbot sessions). See [API Reference → Analytics](/whatomate/api-reference/analytics) for the internal analytics endpoints.
</Aside>
...
WA CRM normalizes granularity to what the Graph API will actually accept for the given range, avoiding Meta-side errors:
...
<Aside type="caution">
  **Per-phone quality rating and messaging-limit tier are not part of Meta Insights.** They are not returned by these analytics endpoints. Those values live on the **account detail** — WA CRM reads a phone number's `quality_rating` (and verification status) when validating account credentials against Meta, and surfaces them under **Settings → Accounts → [account]**. Look there, not here, for a number's current quality rating or tier.
</Aside>
