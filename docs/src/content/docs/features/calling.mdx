...
3. WA CRM establishes a WebRTC peer connection and runs the IVR flow
...
Instead of a static `username`/`credential`, you can point WA CRM at a coturn server running in `use-auth-secret` mode (the TURN REST API). WA CRM then generates a fresh, short-lived credential for every client — no long-lived password is ever handed out.
...
WA CRM uses [Piper](https://github.com/OHF-Voice/piper1-gpl) for offline text-to-speech generation. When admins type greeting text in the IVR flow editor, the server generates OGG/Opus audio files using Piper + `opusenc`. This is optional — you can also upload pre-recorded audio files directly.
