...
WhatsApp Message Templates are pre-approved message formats that allow businesses to send notifications and updates to customers. WA CRM makes it easy to manage, sync, and use templates across your organization.
...
Templates with an `IMAGE`, `VIDEO`, or `DOCUMENT` header require a **sample media file** that Meta will review along with the template text. Without this sample, Meta will reject the template — and WA CRM enforces the same check before submission, so a template with a media header can't be saved or published until you've uploaded one.
...
2. **Publish** — click **Publish**. WA CRM submits the template (including any media handle) to Meta and the status becomes `PENDING`.
...
- WA CRM automatically detects whether your template uses positional or named parameters
...
that WA CRM enforces at template creation time. Both positional (`{{1}}`)
...
WA CRM stores this on each template and refreshes it on every **Sync from
