...
WhatsApp enforces a **24-hour customer service window**. It opens each time the customer sends you a message and lasts 24 hours from that last inbound message. WA CRM tracks this per contact as `service_window_open` (true when the last inbound message was less than 24 hours ago).
...
<Aside type="note">
  This is a WhatsApp Platform rule, not a WA CRM limitation. To re-open a conversation after the window closes, send an approved template; once the customer replies, the free-form window opens again.
</Aside>
...
This flips the contact's incoming messages to `read`. If the sending account has **auto read receipts** enabled, WA CRM also sends read receipts (the blue ticks) back to the customer over the WhatsApp API.
