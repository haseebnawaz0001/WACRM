...
func TestUpdateContactChatbotMessage_SetsTimestampAndResetsReminder(t *testing.T) {
	...
	// Truncate to microseconds: Postgres stores timestamptz at microsecond
	// precision, so a nanosecond-precision baseline can sort after the stored
	// value when both time.Now() calls land in the same microsecond.
